...
public class ForkClient {
    ...
    public ForkClient(ClassLoader loader, Object object, List<String> java, TimeoutLimits timeoutLimits)
        ...
        try {
            ...
            waitForStartBeacon();
            output.writeByte(ForkServer.INIT_PARALLEL_WORKER);
            output.flush();
...
//                throw new IOException("Unexpected byte while waiting for start beacon: "+type);
                log.error("Received {} which was not expected", type);
            }
        }
    }
    ...
    private void sendObject(Object object, List<ForkResource> resources)
            throws IOException {
        log.info("Sending object {} with resources: {}", object.getClass().getSimpleName(), resources.size());
    ...
    private static File createBootstrapJar() throws IOException {
        File file = File.createTempFile("anax-parallel-fork-", ".jar");
        boolean ok = false;
        try {
