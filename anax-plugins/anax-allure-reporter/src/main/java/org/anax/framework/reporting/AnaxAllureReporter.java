...
public class AnaxAllureReporter implements AnaxTestReporter, ReporterSupportsScreenshot, ReporterSupportsVideo {
    ...
    public void endTest(Test test, TestMethod testMethod) {
        ...
        getLifecycle().updateTestCase(testUniqueID, setStatus(getStepStatus(testMethod), testMethod));
    ...
    private Consumer<TestResult> setStatus(final Status status, TestMethod testMethod) {
        ...
        return result -> {
            ...
            } else if(status == Status.PASSED && hasLinkedIssues) {
                log.info("Passed test is has known issue. Changing status to {}", Status.FIXED.value());
                result.setStatus(Status.FIXED);
                result.getSteps().forEach(stepResult -> stepResult.setStatus(Status.FIXED));
            } else {
