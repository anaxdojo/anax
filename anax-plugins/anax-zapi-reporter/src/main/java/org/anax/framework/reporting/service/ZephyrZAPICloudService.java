...
public class ZephyrZAPICloudService implements ZephyrService {
    ...
    @Autowired
    @Qualifier("cache")
    protected LoadingCache<String, DataObject> cache;
    ...
    public String getCycleId(String projectKey, String versionName, String cycleName, boolean initialSearch) {
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + cycleName + "CycleId");
        String cycleId = "";
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            cycleId = dataObject.getData();
            log.info("Got cycle id from cache: {}", cycleId);
        } else {
            ...
            if (cycleInfoFound == null && initialSearch) {//No need to retryconn
                ...
                cache.put(projectKey + versionName + cycleName + "CycleId", new DataObject(cycleInfoFound.getId()));
            }
    ...
    public String getProjectId(String projectKey) {
        String projectId = "";
        DataObject dataObject = cache.getIfPresent(projectKey + "ProjectId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            projectId = dataObject.getData();
            log.info("Got project id from cache: {}", projectId);
        } else {
            try {
                ...
                cache.put(projectKey + "ProjectId", new DataObject(projectId));
            } catch (Exception e) {
    ...
    public String getVersionId(String projectKey, String versionName) {
        String versionId = "";
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + "VersionId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            versionId = dataObject.getData();
            log.info("Get versionId from cache: {}", versionId);
        } else {
            try {
                ...
                cache.put(projectKey + versionName + "VersionId", new DataObject(versionId));
            } catch (Exception e) {
                throw new RetryException(MessageFormat.format("Exception while getting the version id for projectKey {0} and versionName {1}", projectKey, versionName));
            }
        }
    ...
    public JSONObject getIssueExecutionViaAttributeValue(String projectKey, String versionName, String cycleName, String attributeValue) {
        JSONObject issueExecution = null;
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + cycleName + attributeValue + "IssueExecutionJSON");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            try {
                issueExecution = new JSONObject(dataObject.getData());
                log.info("Got issue execution JSON from cache: {}", issueExecution);
            ...
            try {
                ...
                cache.put(projectKey + versionName + cycleName + attributeValue + "IssueExecutionJSON", new DataObject(issueExecution.toString()));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getIssueExecutionIdViaAttributeValue(String projectKey, String versionName, String cycleName, String attributeValue) {
        ...
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + cycleName + attributeValue + "IssueExecutionId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            issueExecutionId = dataObject.getData();
            log.info("Got issue execution id from cache: {}", issueExecutionId);
        } else {
            try {
                issueExecutionId = getIssueExecutionViaAttributeValue(projectKey, versionName, cycleName, attributeValue).get("id").toString();
                cache.put(projectKey + versionName + cycleName + attributeValue + "IssueExecutionId", new DataObject(issueExecutionId));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getIssueExecutionIssueIdViaAttributeValue(String projectKey, String versionName, String cycleName, String attributeValue) {
        ...
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + cycleName + attributeValue + "IssueExecutionIssueId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            issueExecutionIssueId = dataObject.getData();
            log.info("Got issue execution issue id from cache: {}", issueExecutionIssueId);
        } else {
            try {
                ...
                cache.put(projectKey + versionName + cycleName + attributeValue + "IssueExecutionIssueId", new DataObject(issueExecutionIssueId));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getIssueIdViaAttributeValue(String projectKey, String versionName, String cycleName, String attributeValue) {
        String issueId = "";
        DataObject dataObject = cache.getIfPresent(projectKey + versionName + cycleName + attributeValue + "IssueId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            issueId = dataObject.getData();
            log.info("Got issue id from cache: {}", issueId);
        } else {
            String projectId = getProjectId(projectKey);
            String versionId = getVersionId(projectKey, versionName);
            String cycleId = getCycleId(projectKey, versionName, cycleName, false);
            try {
                ...
                cache.put(projectKey + versionName + cycleName + attributeValue + "IssueId", new DataObject(issueId));
            } catch (Exception e) {
                e.printStackTrace();
                log.error("Check !! Issue with this label: {} was not found on project: '{}' at version: '{}' and cycle: '{}'", attributeValue, projectKey, versionName, cycleName);
                throw new RetryException(MessageFormat.format("Error while getting issue id via attribute value, for projectKey {0}, versionName {1}, cycleName {2}, attributeValue {3}", projectKey, versionName, cycleName, attributeValue));
    ...
    public void cloneCycleToVersion(String projectKey, String versionName, CycleClone cycleClone, String originalCycleName) {
        ...
        if (cycleId != null) {
            ...
            ResponseEntity<String> response = restTemplate.exchange(requestUrl, HttpMethod.POST, new HttpEntity(cycleClone, customHttpHeaders.getZapiHeaders(MediaType.APPLICATION_JSON, HttpMethod.POST, requestUrl, zapiUrl)), String.class);
            if (response.getStatusCode() != HttpStatus.OK) {
                throw new RetryException(MessageFormat.format("Failed to clone cycle with name {0} of project {1} from the Unscheduled version to the version {2}", originalCycleName, projectKey, versionName));
            } else {
                try {
                    cache.put(projectKey + versionName + originalCycleName + "CycleId", new DataObject(new JSONObject(response.getBody()).get("id").toString()));
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }
    ...
    private String getJiraIssueId(String issueKey) {
        String issueId = "";
        DataObject dataObject = cache.getIfPresent(issueKey + "JiraIssueId");
        if (dataObject != null && StringUtils.hasLength(dataObject.getData())) {
            issueId = dataObject.getData();
            log.info("Got jira issue id from cache: {}", issueId);
        } else {
            try {
                ...
                if (jiraIssueResponseEntity.getStatusCode() != HttpStatus.OK) {
                    ...
                    cache.put(issueKey + "JiraIssueId", new DataObject(issueId));
                }
            } catch (Exception e) {
